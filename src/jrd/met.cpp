...
static const UCHAR	jrd_101 [118] =
   {	// blr string 
...
static const UCHAR	jrd_118 [190] =
   {	// blr string 
...
static const UCHAR	jrd_141 [128] =
   {	// blr string 
...
static const UCHAR	jrd_152 [109] =
   {	// blr string 
...
static const UCHAR	jrd_159 [126] =
   {	// blr string 
...
static const UCHAR	jrd_180 [116] =
   {	// blr string 
...
static const UCHAR	jrd_190 [230] =
   {	// blr string 
...
static const UCHAR	jrd_208 [159] =
   {	// blr string 
...
static const UCHAR	jrd_236 [115] =
   {	// blr string 
...
static const UCHAR	jrd_245 [119] =
   {	// blr string 
...
static const UCHAR	jrd_255 [104] =
   {	// blr string 
...
static const UCHAR	jrd_263 [108] =
   {	// blr string 
...
/* We need to look up the procedure name in RDB$PROCEDURES */
	...
	{
	if (!request)
	   request = CMP_compile2 (tdbb, (UCHAR*) jrd_278, sizeof(jrd_278), true);
	gds__vtov ((const char*) name.c_str(), (char*) jrd_279.jrd_280, 32);
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_279);
	while (1)
	   {
...
{
   struct {
          SSHORT jrd_276;	// gds__utility 
          SSHORT jrd_277;	// RDB$PROCEDURE_ID 
   } jrd_275;
   struct {
          SSHORT jrd_274;	// RDB$PROCEDURE_ID 
   } jrd_273;
/**************************************
 *
...
/* We need to look up the procedure name in RDB$PROCEDURES */
	...
	{
	if (!request)
	   request = CMP_compile2 (tdbb, (UCHAR*) jrd_272, sizeof(jrd_272), true);
	jrd_273.jrd_274 = id;
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 2, (UCHAR*) &jrd_273);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 4, (UCHAR*) &jrd_275);
	   if (!jrd_275.jrd_276) break;

		if (!REQUEST(irq_l_proc_id))
			REQUEST(irq_l_proc_id) = request;

		procedure = MET_procedure(tdbb, /*P.RDB$PROCEDURE_ID*/
						jrd_275.jrd_277, noscan, flags);
...
{
   struct {
          bid  jrd_259;	// RDB$VIEW_BLR 
          SSHORT jrd_260;	// gds__utility 
          SSHORT jrd_261;	// RDB$FLAGS 
          SSHORT jrd_262;	// RDB$RELATION_ID 
   } jrd_258;
   struct {
          TEXT  jrd_257 [32];	// RDB$RELATION_NAME 
   } jrd_256;
   struct {
          SSHORT jrd_267;	// gds__utility 
          SSHORT jrd_268;	// gds__null_flag 
          SSHORT jrd_269;	// RDB$RELATION_TYPE 
          SSHORT jrd_270;	// RDB$FLAGS 
          SSHORT jrd_271;	// RDB$RELATION_ID 
   } jrd_266;
   struct {
          TEXT  jrd_265 [32];	// RDB$RELATION_NAME 
   } jrd_264;
/**************************************
 *
...
/* We need to look up the relation name in RDB$RELATIONS */
	...
	if (ENCODE_ODS(dbb->dbb_ods_version, dbb->dbb_minor_original) >= ODS_11_1)
	{
		...
		gds__vtov ((const char*) name.c_str(), (char*) jrd_264.jrd_265, 32);
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_264);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 10, (UCHAR*) &jrd_266);
		   if (!jrd_266.jrd_267) break;
	...
	else
	{
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_255, sizeof(jrd_255), true);
		gds__vtov ((const char*) name.c_str(), (char*) jrd_256.jrd_257, 32);
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_256);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 14, (UCHAR*) &jrd_258);
		   if (!jrd_258.jrd_260) break;
...
{
   struct {
          bid  jrd_240;	// RDB$VIEW_BLR 
          TEXT  jrd_241 [32];	// RDB$RELATION_NAME 
          SSHORT jrd_242;	// gds__utility 
          SSHORT jrd_243;	// RDB$FLAGS 
          SSHORT jrd_244;	// RDB$RELATION_ID 
   } jrd_239;
   struct {
          SSHORT jrd_238;	// RDB$RELATION_ID 
   } jrd_237;
   struct {
          TEXT  jrd_249 [32];	// RDB$RELATION_NAME 
          SSHORT jrd_250;	// gds__utility 
          SSHORT jrd_251;	// gds__null_flag 
          SSHORT jrd_252;	// RDB$RELATION_TYPE 
          SSHORT jrd_253;	// RDB$FLAGS 
          SSHORT jrd_254;	// RDB$RELATION_ID 
   } jrd_248;
   struct {
          SSHORT jrd_247;	// RDB$RELATION_ID 
   } jrd_246;
/**************************************
 *
...
/* We need to look up the relation id in RDB$RELATIONS */
	...
	if (ENCODE_ODS(dbb->dbb_ods_version, dbb->dbb_minor_original) >= ODS_11_1)
	{
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_245, sizeof(jrd_245), true);
		jrd_246.jrd_247 = id;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 2, (UCHAR*) &jrd_246);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 42, (UCHAR*) &jrd_248);
		   if (!jrd_248.jrd_250) break;
	...
	else
	{
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_236, sizeof(jrd_236), true);
		jrd_237.jrd_238 = id;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 2, (UCHAR*) &jrd_237);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 46, (UCHAR*) &jrd_239);
		   if (!jrd_239.jrd_242) break;
...
{
   struct {
          bid  jrd_231;	// RDB$TRIGGER_BLR 
          TEXT  jrd_232 [32];	// RDB$TRIGGER_NAME 
          SSHORT jrd_233;	// gds__utility 
          SSHORT jrd_234;	// RDB$FLAGS 
          SSHORT jrd_235;	// RDB$TRIGGER_TYPE 
   } jrd_230;
   struct {
 ...
 **************************************/
	...
	{
	if (!trigger_request)
	   trigger_request = CMP_compile2 (tdbb, (UCHAR*) jrd_227, sizeof(jrd_227), true);
	gds__vtov ((const char*) relation->rel_name.c_str(), (char*) jrd_228.jrd_229, 32);
	EXE_start (tdbb, trigger_request, dbb->dbb_sys_trans);
	EXE_send (tdbb, trigger_request, 0, 32, (UCHAR*) &jrd_228);
	while (1)
	   {
	   EXE_receive (tdbb, trigger_request, 1, 46, (UCHAR*) &jrd_230);
	   if (!jrd_230.jrd_233) break;
...
{
   struct {
          bid  jrd_224;	// RDB$TRANSACTION_DESCRIPTION 
          SLONG  jrd_225;	// RDB$TRANSACTION_ID 
          SSHORT jrd_226;	// RDB$TRANSACTION_STATE 
   } jrd_223;
/**************************************
 *
 ...
 **************************************/
	...
	/*STORE(REQUEST_HANDLE request) X IN RDB$TRANSACTIONS*/
	{
		...
		jrd_223.jrd_225 = transaction->tra_number;
		/*X.RDB$TRANSACTION_STATE*/
		jrd_223.jrd_226 = /*RDB$TRANSACTIONS.RDB$TRANSACTION_STATE.LIMBO*/
   1;
	...
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 14, (UCHAR*) &jrd_223);
	}
...
{
   struct {
          SSHORT jrd_170;	// gds__utility 
   } jrd_169;
   struct {
          SSHORT jrd_167;	// gds__null_flag 
          SSHORT jrd_168;	// RDB$VALID_BLR 
   } jrd_166;
   struct {
          SSHORT jrd_163;	// gds__utility 
          SSHORT jrd_164;	// gds__null_flag 
          SSHORT jrd_165;	// RDB$VALID_BLR 
   } jrd_162;
   struct {
          SSHORT jrd_161;	// RDB$PROCEDURE_ID 
   } jrd_160;
   struct {
          bid  jrd_175;	// RDB$DEBUG_INFO 
          SSHORT jrd_176;	// gds__utility 
          SSHORT jrd_177;	// gds__null_flag 
          SSHORT jrd_178;	// gds__null_flag 
          SSHORT jrd_179;	// RDB$PROCEDURE_TYPE 
   } jrd_174;
   struct {
          SSHORT jrd_173;	// RDB$PROCEDURE_ID 
   } jrd_172;
   struct {
          ...
          SSHORT jrd_189;	// RDB$COLLATION_ID 
   } jrd_184;
   struct {
          TEXT  jrd_182 [32];	// RDB$PARAMETER_NAME 
          TEXT  jrd_183 [32];	// RDB$PROCEDURE_NAME 
   } jrd_181;
   struct {
          bid  jrd_194;	// RDB$DEFAULT_VALUE 
          TEXT  jrd_195 [32];	// RDB$FIELD_NAME 
          TEXT  jrd_196 [32];	// RDB$PARAMETER_NAME 
          TEXT  jrd_197 [32];	// RDB$PROCEDURE_NAME 
          SSHORT jrd_198;	// gds__utility 
          SSHORT jrd_199;	// gds__null_flag 
          SSHORT jrd_200;	// RDB$COLLATION_ID 
          SSHORT jrd_201;	// RDB$CHARACTER_SET_ID 
          SSHORT jrd_202;	// RDB$FIELD_SUB_TYPE 
          SSHORT jrd_203;	// RDB$FIELD_LENGTH 
          SSHORT jrd_204;	// RDB$FIELD_SCALE 
          SSHORT jrd_205;	// RDB$FIELD_TYPE 
          SSHORT jrd_206;	// RDB$PARAMETER_NUMBER 
          SSHORT jrd_207;	// RDB$PARAMETER_TYPE 
   } jrd_193;
   struct {
          TEXT  jrd_192 [32];	// RDB$PROCEDURE_NAME 
   } jrd_191;
   struct {
          bid  jrd_212;	// RDB$PROCEDURE_BLR 
          TEXT  jrd_213 [32];	// RDB$SECURITY_CLASS 
          TEXT  jrd_214 [32];	// RDB$PROCEDURE_NAME 
          SSHORT jrd_215;	// gds__utility 
          SSHORT jrd_216;	// RDB$PROCEDURE_OUTPUTS 
          SSHORT jrd_217;	// RDB$PROCEDURE_INPUTS 
          SSHORT jrd_218;	// gds__null_flag 
          SSHORT jrd_219;	// RDB$SYSTEM_FLAG 
          SSHORT jrd_220;	// gds__null_flag 
          SSHORT jrd_221;	// RDB$PROCEDURE_ID 
   } jrd_211;
   struct {
          SSHORT jrd_210;	// RDB$PROCEDURE_ID 
   } jrd_209;
/**************************************
 *
 ...
 **************************************/
	...
	if (!noscan)
	{
		...
		{
		if (!request)
		   request = CMP_compile2 (tdbb, (UCHAR*) jrd_208, sizeof(jrd_208), true);
		jrd_209.jrd_210 = procedure->prc_id;
		EXE_start (tdbb, request, dbb->dbb_sys_trans);
		EXE_send (tdbb, request, 0, 2, (UCHAR*) &jrd_209);
		while (1)
		   {
		   EXE_receive (tdbb, request, 1, 86, (UCHAR*) &jrd_211);
		   if (!jrd_211.jrd_215) break;
			...
			if (/*P.RDB$SYSTEM_FLAG.NULL*/
			    jrd_211.jrd_218 || !/*P.RDB$SYSTEM_FLAG*/
     jrd_211.jrd_219)
			{
			...
			{
				procedure->prc_input_fields->resize(procedure->prc_inputs);
			}
			...
			{
				procedure->prc_output_fields->resize(procedure->prc_outputs);
			}
			...
			{
			if (!request2)
			   request2 = CMP_compile2 (tdbb, (UCHAR*) jrd_190, sizeof(jrd_190), true);
			gds__vtov ((const char*) jrd_211.jrd_214, (char*) jrd_191.jrd_192, 32);
			EXE_start (tdbb, request2, dbb->dbb_sys_trans);
			EXE_send (tdbb, request2, 0, 32, (UCHAR*) &jrd_191);
			while (1)
			   {
			   EXE_receive (tdbb, request2, 1, 124, (UCHAR*) &jrd_193);
			   if (!jrd_193.jrd_198) break;
				...
				if (ENCODE_ODS(dbb->dbb_ods_version, dbb->dbb_minor_original) >= ODS_11_1)
				{
					...
					{
					if (!request3)
					   request3 = CMP_compile2 (tdbb, (UCHAR*) jrd_180, sizeof(jrd_180), true);
					gds__vtov ((const char*) jrd_193.jrd_196, (char*) jrd_181.jrd_182, 32);
					gds__vtov ((const char*) jrd_193.jrd_197, (char*) jrd_181.jrd_183, 32);
					EXE_start (tdbb, request3, dbb->dbb_sys_trans);
					EXE_send (tdbb, request3, 0, 64, (UCHAR*) &jrd_181);
					while (1)
					   {
					   EXE_receive (tdbb, request3, 1, 16, (UCHAR*) &jrd_184);
				...
				DSC_make_descriptor(&parameter->prm_desc, /*F.RDB$FIELD_TYPE*/
									  jrd_193.jrd_205,
									/*F.RDB$FIELD_SCALE*/
									jrd_193.jrd_204, /*F.RDB$FIELD_LENGTH*/
  jrd_193.jrd_203,
									/*F.RDB$FIELD_SUB_TYPE*/
									jrd_193.jrd_202, /*F.RDB$CHARACTER_SET_ID*/
  jrd_193.jrd_201,
									(pa_collation_id_null ? /*F.RDB$COLLATION_ID*/
												jrd_193.jrd_200 : pa_collation_id));

				if (/*PA.RDB$PARAMETER_TYPE*/
				    jrd_193.jrd_207 == 0 &&
					(!pa_default_value_null ||
					 (fb_utils::implicit_domain(/*F.RDB$FIELD_NAME*/
								    jrd_193.jrd_195) && !/*F.RDB$DEFAULT_VALUE.NULL*/
      jrd_193.jrd_199)))
				{
...

			{
				...
				if (ENCODE_ODS(dbb->dbb_ods_version, dbb->dbb_minor_original) >= ODS_11_1)
				{
					...
					{
					if (!request4)
					   request4 = CMP_compile2 (tdbb, (UCHAR*) jrd_171, sizeof(jrd_171), true);
					jrd_172.jrd_173 = procedure->prc_id;
					EXE_start (tdbb, request4, dbb->dbb_sys_trans);
					EXE_send (tdbb, request4, 0, 2, (UCHAR*) &jrd_172);
					while (1)
					   {
					   EXE_receive (tdbb, request4, 1, 16, (UCHAR*) &jrd_174);
					   if (!jrd_174.jrd_176) break;
						...
						if (!/*PT.RDB$DEBUG_INFO.NULL*/
							...
							DBG_parse_debug_info(tdbb, &/*PT.RDB$DEBUG_INFO*/
										    jrd_174.jrd_175, csb->csb_dbg_info);
			...
			!(dbb->dbb_flags & DBB_read_only) && !valid_blr)
		{
			...
			{
			if (!request5)
			   request5 = CMP_compile2 (tdbb, (UCHAR*) jrd_159, sizeof(jrd_159), true);
			jrd_160.jrd_161 = procedure->prc_id;
			EXE_start (tdbb, request5, dbb->dbb_sys_trans);
			EXE_send (tdbb, request5, 0, 2, (UCHAR*) &jrd_160);
			while (1)
			   {
			   EXE_receive (tdbb, request5, 1, 6, (UCHAR*) &jrd_162);
			   if (!jrd_162.jrd_163) break;
				...
				/*MODIFY P USING*/
				{
					...
					/*P.RDB$VALID_BLR*/
					jrd_162.jrd_165 = TRUE;
					/*P.RDB$VALID_BLR.NULL*/
					jrd_162.jrd_164 = FALSE;
				/*END_MODIFY;*/
				jrd_166.jrd_167 = jrd_162.jrd_164;
				jrd_166.jrd_168 = jrd_162.jrd_165;
				EXE_send (tdbb, request5, 2, 4, (UCHAR*) &jrd_166);
				}
			/*END_FOR;*/
			   EXE_send (tdbb, request5, 3, 2, (UCHAR*) &jrd_169);
			   }
			}
				...
				const TEXT* privilege)
{
   ...
   struct {
          TEXT  jrd_143 [32];	// RDB$GRANTOR 
          TEXT  jrd_144 [32];	// RDB$RELATION_NAME 
          TEXT  jrd_145 [7];	// RDB$PRIVILEGE 
   } jrd_142;
   struct {
...
/* See if the revokee still has the privilege.  If so, there's
   nothing to do */
	...
	{
	if (!request)
	   request = CMP_compile2 (tdbb, (UCHAR*) jrd_152, sizeof(jrd_152), true);
	gds__vtov ((const char*) revokee, (char*) jrd_153.jrd_154, 32);
	gds__vtov ((const char*) relation, (char*) jrd_153.jrd_155, 32);
	gds__vtov ((const char*) privilege, (char*) jrd_153.jrd_156, 7);
	EXE_start (tdbb, request, transaction);
	EXE_send (tdbb, request, 0, 71, (UCHAR*) &jrd_153);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 2, (UCHAR*) &jrd_157);
	   if (!jrd_157.jrd_158) break;
...
/* User lost privilege.  Take it away from anybody he/she gave
   it to. */
	...
	gds__vtov ((const char*) revokee, (char*) jrd_142.jrd_143, 32);
	gds__vtov ((const char*) relation, (char*) jrd_142.jrd_144, 32);
	gds__vtov ((const char*) privilege, (char*) jrd_142.jrd_145, 7);
	EXE_start (tdbb, request, transaction);
	EXE_send (tdbb, request, 0, 71, (UCHAR*) &jrd_142);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 2, (UCHAR*) &jrd_146);
	   if (!jrd_146.jrd_147) break;

		if (!REQUEST(irq_revoke2))
			REQUEST(irq_revoke2) = request;
		/*ERASE P;*/
		EXE_send (tdbb, request, 2, 2, (UCHAR*) &jrd_148);
	/*END_FOR;*/
	   EXE_send (tdbb, request, 3, 2, (UCHAR*) &jrd_150);
	   }
	}
 ...
 *
 * Functional description
 *      Wrap scan_partners() with dbb_meta_mutex.
 *
 **************************************/
...
void MET_scan_relation(thread_db* tdbb, jrd_rel* relation)
{
   struct {
          ...
          bid  jrd_123;	// RDB$RUNTIME 
          TEXT  jrd_124 [256];	// RDB$EXTERNAL_FILE 
          bid  jrd_125;	// RDB$VIEW_BLR 
          TEXT  jrd_126 [32];	// RDB$OWNER_NAME 
          TEXT  jrd_127 [32];	// RDB$RELATION_NAME 
          TEXT  jrd_128 [32];	// RDB$SECURITY_CLASS 
          SSHORT jrd_129;	// gds__utility 
          SSHORT jrd_130;	// gds__null_flag 
          SSHORT jrd_131;	// gds__null_flag 
          SSHORT jrd_132;	// RDB$FIELD_ID 
          SSHORT jrd_133;	// RDB$FORMAT 
   } jrd_121;
   struct {
          SSHORT jrd_120;	// RDB$RELATION_ID 
   } jrd_119;
   struct {
          SSHORT jrd_138;	// gds__utility 
          SSHORT jrd_139;	// gds__null_flag 
          SSHORT jrd_140;	// RDB$RELATION_TYPE 
   } jrd_137;
   struct {
          SSHORT jrd_136;	// RDB$RELATION_ID 
   } jrd_135;
/**************************************
 *
...
/* If anything errors, catch it to reset the scan flag.  This will
   ...
   again. */
	...
	if (ENCODE_ODS(dbb->dbb_ods_version, dbb->dbb_minor_original) >= ODS_11_1)
	{
		...
		{
		if (!sub_request)
		   sub_request = CMP_compile2 (tdbb, (UCHAR*) jrd_134, sizeof(jrd_134), true);
		jrd_135.jrd_136 = relation->rel_id;
		EXE_start (tdbb, sub_request, dbb->dbb_sys_trans);
		EXE_send (tdbb, sub_request, 0, 2, (UCHAR*) &jrd_135);
		while (1)
		   {
		   EXE_receive (tdbb, sub_request, 1, 6, (UCHAR*) &jrd_137);
		   if (!jrd_137.jrd_138) break;
...
/* Since this can be called recursively, find an inactive clone of the request */
	...
	jrd_119.jrd_120 = relation->rel_id;
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 2, (UCHAR*) &jrd_119);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 410, (UCHAR*) &jrd_121);
	   if (!jrd_121.jrd_129) break;
			...
			else
			{
				relation->rel_view_rse =
					(RecordSelExpr*) MET_parse_blob(tdbb, relation,
											&/*REL.RDB$VIEW_BLR*/
											 jrd_121.jrd_125, csb, NULL, false);
			}
		    ...
		    jrd_121.jrd_124[0])
		{
			EXT_file(relation, /*REL.RDB$EXTERNAL_FILE*/
					   jrd_121.jrd_124); //, &REL.RDB$EXTERNAL_DESCRIPTION);
		}
...
{
   struct {
          TEXT  jrd_116 [1024];	// RDB$MESSAGE 
          SSHORT jrd_117;	// gds__utility 
   } jrd_115;
   struct {
          TEXT  jrd_113 [32];	// RDB$TRIGGER_NAME 
          SSHORT jrd_114;	// RDB$MESSAGE_NUMBER 
   } jrd_112;
/**************************************
 *
 ...
 **************************************/
	...
	gds__vtov ((const char*) name.c_str(), (char*) jrd_112.jrd_113, 32);
	jrd_112.jrd_114 = number;
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 34, (UCHAR*) &jrd_112);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 1026, (UCHAR*) &jrd_115);
	   if (!jrd_115.jrd_117) break;
...
void MET_update_shadow(thread_db* tdbb, Shadow* shadow, USHORT file_flags)
{
   ...
   struct {
          SSHORT jrd_108;	// RDB$FILE_FLAGS 
   } jrd_107;
   struct {
          SSHORT jrd_105;	// gds__utility 
          SSHORT jrd_106;	// RDB$FILE_FLAGS 
   } jrd_104;
   struct {
          SSHORT jrd_103;	// RDB$SHADOW_NUMBER 
   } jrd_102;
/**************************************
 *
 ...
 **************************************/
	...
	{
	if (!handle)
	   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_101, sizeof(jrd_101), true);
	jrd_102.jrd_103 = shadow->sdw_number;
	EXE_start (tdbb, handle, dbb->dbb_sys_trans);
	EXE_send (tdbb, handle, 0, 2, (UCHAR*) &jrd_102);
	while (1)
	   {
	   EXE_receive (tdbb, handle, 1, 4, (UCHAR*) &jrd_104);
	   if (!jrd_104.jrd_105) break;
		/*MODIFY FIL USING*/
		{
		
			/*FIL.RDB$FILE_FLAGS*/
			jrd_104.jrd_106 = file_flags;
		/*END_MODIFY;*/
		jrd_107.jrd_108 = jrd_104.jrd_106;
		EXE_send (tdbb, handle, 2, 2, (UCHAR*) &jrd_107);
		}
	/*END_FOR;*/
	   EXE_send (tdbb, handle, 3, 2, (UCHAR*) &jrd_109);
	   }
	}
	CMP_release(tdbb, handle);
}
...
void MET_update_transaction(thread_db* tdbb, jrd_tra* transaction, const bool do_commit)
{
   ...
   struct {
          SSHORT jrd_96;	// RDB$TRANSACTION_STATE 
   } jrd_95;
   struct {
          SSHORT jrd_93;	// gds__utility 
          SSHORT jrd_94;	// RDB$TRANSACTION_STATE 
   } jrd_92;
   struct {
          SLONG  jrd_91;	// RDB$TRANSACTION_ID 
   } jrd_90;
/**************************************
 *
 ...
 **************************************/
	...
	jrd_90.jrd_91 = transaction->tra_number;
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 4, (UCHAR*) &jrd_90);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 4, (UCHAR*) &jrd_92);
	   if (!jrd_92.jrd_93) break;
		...
		{
			/*ERASE X*/
			EXE_send (tdbb, request, 4, 2, (UCHAR*) &jrd_99);
		}
		else
		{
			/*MODIFY X*/
			{
				...
				/*X.RDB$TRANSACTION_STATE*/
				jrd_92.jrd_94 = do_commit ?
					/*RDB$TRANSACTIONS.RDB$TRANSACTION_STATE.COMMITTED*/
					2 :
					/*RDB$TRANSACTIONS.RDB$TRANSACTION_STATE.ROLLED_BACK*/
					3;
			/*END_MODIFY;*/
			jrd_95.jrd_96 = jrd_92.jrd_94;
			EXE_send (tdbb, request, 2, 2, (UCHAR*) &jrd_95);
			}
		}
	/*END_FOR;*/
	   EXE_send (tdbb, request, 3, 2, (UCHAR*) &jrd_97);
	   }
	}
...
static bool get_type(thread_db* tdbb, USHORT* id, const UCHAR* name, const TEXT* field)
{
   struct {
          ...
          SSHORT jrd_88;	// RDB$TYPE 
   } jrd_86;
   struct {
          TEXT  jrd_84 [32];	// RDB$TYPE_NAME 
...
/* Try for exact name match */
	...
	{
	if (!handle)
	   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_82, sizeof(jrd_82), true);
	gds__vtov ((const char*) buffer, (char*) jrd_83.jrd_84, 32);
	gds__vtov ((const char*) field, (char*) jrd_83.jrd_85, 32);
	EXE_start (tdbb, handle, dbb->dbb_sys_trans);
	EXE_send (tdbb, handle, 0, 64, (UCHAR*) &jrd_83);
	while (1)
	   {
	   EXE_receive (tdbb, handle, 1, 4, (UCHAR*) &jrd_86);
	   if (!jrd_86.jrd_87) break;
...
static void lookup_view_contexts( thread_db* tdbb, jrd_rel* view)
{
   struct {
          ...
          TEXT  jrd_79 [256];	// RDB$CONTEXT_NAME 
          SSHORT jrd_80;	// gds__utility 
          SSHORT jrd_81;	// RDB$VIEW_CONTEXT 
   } jrd_77;
   struct {
          TEXT  jrd_76 [32];	// RDB$VIEW_NAME 
   } jrd_75;
/**************************************
 *
 ...
 **************************************/
	...
	{
	if (!request)
	   request = CMP_compile2 (tdbb, (UCHAR*) jrd_74, sizeof(jrd_74), true);
	gds__vtov ((const char*) view->rel_name.c_str(), (char*) jrd_75.jrd_76, 32);
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_75);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 292, (UCHAR*) &jrd_77);
										  ...
										  const UCHAR* collation)
{
   struct {
          ...
          SSHORT jrd_59;	// RDB$COLLATION_ID 
          SSHORT jrd_60;	// RDB$CHARACTER_SET_ID 
   } jrd_57;
   struct {
          TEXT  jrd_55 [32];	// RDB$COLLATION_NAME 
          TEXT  jrd_56 [32];	// RDB$TYPE_NAME 
   } jrd_54;
   struct {
          SSHORT jrd_65;	// gds__utility 
          SSHORT jrd_66;	// RDB$COLLATION_ID 
          SSHORT jrd_67;	// RDB$CHARACTER_SET_ID 
   } jrd_64;
   struct {
   ...
   struct {
          ...
          SSHORT jrd_73;	// RDB$CHARACTER_SET_ID 
   } jrd_71;
   struct {
          TEXT  jrd_70 [32];	// RDB$CHARACTER_SET_NAME 
   } jrd_69;
/**************************************
 *
 ...
 **************************************/
	...
	if (collation == NULL)
	{
		...
		{
		if (!handle)
		   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_68, sizeof(jrd_68), true);
		gds__vtov ((const char*) charset, (char*) jrd_69.jrd_70, 32);
		EXE_start (tdbb, handle, dbb->dbb_sys_trans);
		EXE_send (tdbb, handle, 0, 32, (UCHAR*) &jrd_69);
		while (1)
		   {
		   EXE_receive (tdbb, handle, 1, 4, (UCHAR*) &jrd_71);
	...
	if (charset == NULL)
	{
		...
		EXE_start (tdbb, handle, dbb->dbb_sys_trans);
		EXE_send (tdbb, handle, 0, 32, (UCHAR*) &jrd_62);
		while (1)
		   {
		   EXE_receive (tdbb, handle, 1, 6, (UCHAR*) &jrd_64);
		   if (!jrd_64.jrd_65) break;

			found = true;
			*id = /*COL.RDB$CHARACTER_SET_ID*/
			      jrd_64.jrd_67 | (/*COL.RDB$COLLATION_ID*/
    jrd_64.jrd_66 << 8);
	...
	{
	if (!handle)
	   handle = CMP_compile2 (tdbb, (UCHAR*) jrd_53, sizeof(jrd_53), true);
	gds__vtov ((const char*) collation, (char*) jrd_54.jrd_55, 32);
	gds__vtov ((const char*) charset, (char*) jrd_54.jrd_56, 32);
	EXE_start (tdbb, handle, dbb->dbb_sys_trans);
	EXE_send (tdbb, handle, 0, 64, (UCHAR*) &jrd_54);
	while (1)
	   {
	   EXE_receive (tdbb, handle, 1, 6, (UCHAR*) &jrd_57);
	   if (!jrd_57.jrd_58) break;

		found = true;
		*id = /*CS.RDB$CHARACTER_SET_ID*/
		      jrd_57.jrd_60 | (/*COL.RDB$COLLATION_ID*/
    jrd_57.jrd_59 << 8);
...
static void scan_partners(thread_db* tdbb, jrd_rel* relation)
{
   struct {
          ...
          SSHORT jrd_38;	// RDB$INDEX_ID 
          SSHORT jrd_39;	// RDB$INDEX_ID 
          SSHORT jrd_40;	// RDB$INDEX_INACTIVE 
          SSHORT jrd_41;	// RDB$INDEX_INACTIVE 
   } jrd_35;
   struct {
          TEXT  jrd_34 [32];	// RDB$RELATION_NAME 
   } jrd_33;
   struct {
          TEXT  jrd_47 [32];	// RDB$RELATION_NAME 
          SSHORT jrd_48;	// gds__utility 
          SSHORT jrd_49;	// RDB$INDEX_ID 
          SSHORT jrd_50;	// RDB$INDEX_ID 
          SSHORT jrd_51;	// RDB$INDEX_INACTIVE 
          SSHORT jrd_52;	// RDB$INDEX_INACTIVE 
   } jrd_46;
   struct {
          TEXT  jrd_44 [32];	// RDB$RELATION_NAME 
          TEXT  jrd_45 [12];	// RDB$CONSTRAINT_TYPE 
   } jrd_43;
/**************************************
 *
 ...
 *
 * Functional description
 *      Scan relation constraints for partner indices participating 
 *      in a foreign key relationships.
 *
 **************************************/

	fb_assert(relation->rel_flags & REL_check_partners);
	Database* dbb = tdbb->getDatabase();
	...
	gds__vtov ((const char*) relation->rel_name.c_str(), (char*) jrd_43.jrd_44, 32);
	gds__vtov ((const char*) FOREIGN_KEY, (char*) jrd_43.jrd_45, 12);
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 44, (UCHAR*) &jrd_43);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 42, (UCHAR*) &jrd_46);
	   if (!jrd_46.jrd_48) break;
		...
		if (partner_relation && !/*IDX.RDB$INDEX_INACTIVE*/
					 jrd_46.jrd_52 && !/*IND.RDB$INDEX_INACTIVE*/
     jrd_46.jrd_51)
		{
	...
	gds__vtov ((const char*) relation->rel_name.c_str(), (char*) jrd_33.jrd_34, 32);
	EXE_start (tdbb, request, dbb->dbb_sys_trans);
	EXE_send (tdbb, request, 0, 32, (UCHAR*) &jrd_33);
	while (1)
	   {
	   EXE_receive (tdbb, request, 1, 42, (UCHAR*) &jrd_35);
	   if (!jrd_35.jrd_37) break;
		...
		// System relations have no foreign keys thus can't have dependent relations.
		// If this will change, fix also IDX_modify_flag_uk_modified().
		...
		if (partner_relation && !/*IDX.RDB$INDEX_INACTIVE*/
					 jrd_35.jrd_41 && !/*IND.RDB$INDEX_INACTIVE*/
     jrd_35.jrd_40)
		{
